...
module.exports = class {
  constructor(client) {
    ...
    const resourcesNick   = client.netConfig.resources_nick;
    const self            = this;

    client.irc.on('disconnect', () => {
      self._pendingPackets.clear();

      while(self._pendingExecutes.length) {
        let key = self._pendingPackets.keys().next().value;
        self._pendingExecutes.get(key).error('DISCONNECTED');
      }
    });

    client.irc.on('message', (from, to, text) => {
      if(!from.startsWith(resourcesNick)) { return; }
      ...
      if(cmd === 'init') {
        let pending = new PendingPacket(id, (data) => {
          if(!data.id) { return logger.error(data); }
          this._pendingExecutes.get(data.id).data(data);
        }, () => self._pendingPackets.delete(id));

        pending.init(payload);
    ...
    this._send = function(msg) {
      self._irc.say(resourcesNick, msg);
    };
  }
  ...
  execute(req, cb) {
    ...
    this._pendingExecutes.set(id, new PendingExecute(id, cb, () => self._pendingPackets.delete(id)));
  }
