...
class RemoteObject {
  ...
  change(newNick) {
    ...
    if(data.id !== this._id) {
      logger.error('object \'' + this._id + '\' has changed id to \'' + data.id + '\', ignoring (but this lead to sync problems)');
    }
    ...
    for(let name of Object.keys(this._properties)) {
      ...
      if(newValue === undefined) {
        logger.error('object \'' + this._id + '\' has lost property \'' + name + '\', ignoring (but this lead to sync problems)');
        continue;
      }

      if(oldValue !== newValue) {
        this._properties[name] = newValue;
        this._emitter.emit('change', this._id, name);
      }
    }
