...
class Session extends EventEmitter {
  constructor(id, socket, netConfig) {
    super();
    ...
    this._socket.on('action', (data) => {
      this._net.action(data.id, data.name, data.args);
    });

    // TODO : send repo state and register to repo changes
  }
...
module.exports = class {
  constructor(netConfig, httpPort) {
    ...
    this.netRepository      = new Net.Repository(this._netAgent);
  ...
  _createSession(socket) {
    ...
    const session = new Session(id, socket, this._netConfig);
