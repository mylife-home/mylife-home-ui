...
class Session extends EventEmitter {
  constructor(id, socket, netConfig, netRepository) {
    ...
    this._id              = id;
    this._socket          = socket;
    this._net             = new common.net.Client(netConfig, this._createNick());
...
module.exports = class {
  constructor(config) {
    ...
    this._netAgent         = new common.net.Client(netConfig, 'ui-agent');
    this._netRepository    = new common.net.Repository(this._netAgent);
    ...
    this._webServer        = new web.Server(this.netRepository, this._netJpacketClient,  this._createSession.bind(this), config.web.port);
  ...
  _executeSessionList(req, cb) {
    ...
    return setImmediate(cb, factory.createSessionList(list));
  }

  _executeSessionKill(req, cb) {
    const factory = common.net.jpacket.Factory;
    const session = this._sessions.get(req.sessionId);
    if(!session) { return setImmediate(cb, factory.createError('Session not found:' + req.sessionId)); }
    session.kill(() => setImmediate(cb, factory.createSuccess()));
  }
